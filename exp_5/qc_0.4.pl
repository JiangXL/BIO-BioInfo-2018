...
open result, '>HomoExonReads_qcseq_gc_0.4';
...
open QC_result, '>HomoExonReads_qc_base';


#print  "QC_base:".$qc_base[0]/($counter_seq+1);

$qc_base_ref=\@qc_base;

foreach $qc (@$qc_base_ref){
	print QC_result "QC_base:".$qc/($counter_seq+1)."\n";
}


my $graph_gc = GD::Graph::histogram->new(1024,768);
$graph_gc->set(
	x_label  => 'Sequence',
	y_label  => 'GC Content',
	title    => 'GC Content of HomoExon',
)or die $graph_gc -> error;

my $gd = $graph_gc->plot(\@gc_seq) or die $graph_gc -> error ;

open(IMG, '>graph_gc.jpeg') or die $!;
binmode IMG;
print IMG $gd -> jpeg;
